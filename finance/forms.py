...
class ReservationForm(forms.ModelForm):
    ...
    def clean(self, *args, **kwargs):
        ...
        date_list = []
        booked_dates = [
            pickup_date + datetime.timedelta(days=x)
            for x in range((return_date - pickup_date).days)
        ]

        for reservation in reservations:
            date_list += [
                reservation.pickup_date + datetime.timedelta(days=x)
                for x in range(
                    1, (reservation.return_date - reservation.pickup_date).days
                )
            ]

        if return_date < pickup_date:
            raise ValidationError(
                "Data e kthimit nuk duhet te jete me e vogel se data e marrjes"
            )
        ...
        for date in booked_dates:
            if date in date_list:
                raise ValidationError(
                    f"Data me afert lire per kthim te kesaj veture eshte {min(date_list)}"
                )
